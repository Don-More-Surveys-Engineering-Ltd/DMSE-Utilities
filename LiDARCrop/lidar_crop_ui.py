...
from LiDARCrop.lidar_crop import crop_lidar_files
...
class LiDARCropSection(ttk.Frame):
    ...
    def on_compute(self) -> None:
        ...
        inputs = [Path(line) for line in self.form_data.input_files.get().splitlines()]
        output = Path(self.form_data.output_file.get())
        retry = False
        # create output
        try:
            crop_lidar_files(
        ...
        messagebox.showinfo(
            "Success",
            f"Output saved to {self.form_data.output_file.get()}.",
        )
